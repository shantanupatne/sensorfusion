...
void clusterKptMatchesWithROI(BoundingBox &boundingBox, vector<cv::KeyPoint> &kptsPrev, vector<cv::KeyPoint> &kptsCurr, vector<cv::DMatch> &kptMatches)
{
    ...
    for (auto& match: kptMatches) {
        ...
        if (boundingBox.roi.contains(currKp.pt)) {
            ...
            avgDist += cv::norm(currKp.pt - prevKp.pt);
        }
    }
                     ...
                     vector<LidarPoint> &lidarPointsCurr, double frameRate, double &TTC)
{
    ...
    vector<double> validXPrev, validXCurr;
    for (auto it = lidarPointsPrev.begin(); it != lidarPointsPrev.end(); ++it)
    {
        
        if (abs(it->y) <= laneWidth / 2.0)
        { 
            validXPrev.push_back(it->x);
        }
    }
    ...
    for (auto it = lidarPointsCurr.begin(); it != lidarPointsCurr.end(); ++it)
    {
        ...
        if (abs(it->y) <= laneWidth / 2.0)
        { 
            validXCurr.push_back(it->x);
        }
    }

    if (validXPrev.empty() || validXCurr.empty()) {
        TTC = NAN;
        return;
    }
    std::sort(validXPrev.begin(), validXPrev.end());
    std::sort(validXCurr.begin(), validXCurr.end());
...
void matchBoundingBoxes(vector<cv::DMatch> &matches, map<int, int> &bbBestMatches, DataFrame &prevFrame, DataFrame &currFrame)
{
    ...
    // create empty grid
    vector<vector<int>> pointCount(prevBoxCount, vector<int> (currBoxCount, 0));
    ...
    for (auto& match : matches) {
        ...
        cv::Point prevPoint = prevkp.pt;
        cv::Point currPoint = currkp.pt;

        vector<int> prevIndices, currIndices;

        for (int i = 0, j = 0; i < prevBoxCount || j < currBoxCount;) {
            if (i < prevBoxCount) {
                if(prevFrame.boundingBoxes[i].roi.contains(prevPoint)) {
                    prevIndices.push_back(i);
                }
                ++i;
            }
            ...
            if (j < currBoxCount) {
                if (currFrame.boundingBoxes[j].roi.contains(currPoint)) {
                    currIndices.push_back(j);
                }
                ++j;
            }
        }

        if (prevIndices.size() > 0 && currIndices.size() > 0) 
        {
            for (auto& pIdx : prevIndices) {
                for (auto& cIdx: currIndices) {
                    pointCount[pIdx][cIdx] += 1;
                }
            }
        ...
        for (int i = 0; i < prevBoxCount; i++) 
        {
            ...
            {
                if (pointCount[i][j] > maxCount) {
                    maxCount = pointCount[i][j];
                    maxIdx = j;
                }
            }
