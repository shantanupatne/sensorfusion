...
public:
	...
	std::vector<double> NIS_lidar, NIS_radar;
	// --------------------------------
	...
	void stepHighway(double egoVelocity, long long timestamp, int frame_per_sec, pcl::visualization::PCLVisualizer::Ptr& viewer)
	{
		...
		egoCar.render(viewer);
		std::vector<double> nis_meas_lidar, nis_meas_radar;
		...
		for (int i = 0; i < traffic.size(); i++)
		{
			...
			if(trackCars[i])
			{
				...
				nis_meas_lidar.push_back(traffic[i].ukf.nis_lidar);
				nis_meas_radar.push_back(traffic[i].ukf.nis_radar);

			}
		}
		double nis_l_ = std::accumulate(nis_meas_lidar.begin(), nis_meas_lidar.end(), 0.);
		double nis_r_ = std::accumulate(nis_meas_radar.begin(), nis_meas_radar.end(), 0.);

		NIS_lidar.push_back(nis_l_ / nis_meas_lidar.size());
		NIS_radar.push_back(nis_r_ / nis_meas_radar.size());
